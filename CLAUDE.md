...
| Часть | Что |
|---|---|
| backend | Django 5.2 + DRF, БД — **PostgreSQL** (psycopg 3) |
| frontend | Vue 3 + Vite + Tailwind, SPA + пререндер статики на 3 языка (uz/ru/en) |
| админка | код есть, **наружу не отдаётся** (`SERVICE_ROLE=api`); при нужде поднимается на localhost через SSH-туннель |
...
- **Mongo больше нет.** Проект родился на `django-mongodb-backend`; переехали на
  Postgres 2026-09-13 (данные были насквозь реляционные, а Mongo отбирала
  транзакции, ломала `icontains`, не давала simplejwt-блэклист и вытаскивала
  ObjectId в API). Контейнер `mars-it-school-mongo` снесён. Если в истории
  гита или в доках интернов встречается `MONGODB_URI` — это прошлое.
- **`id` в API теперь целое число**, не строка. Фронт на тип не завязан.
- **Контент от интернов** приедет в их формате — `mongodump` бесполезен.
  Просить `manage.py dumpdata`; ObjectId→int придётся перемапить импортом.
...
- **Резюме кандидатов — персональные данные.** Лежат вне `MEDIA_ROOT`. Раньше
  скачивались через админку; с `SERVICE_ROLE=api` до них только через shell.
