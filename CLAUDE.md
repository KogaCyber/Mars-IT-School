...
| админка | **не используется.** `SERVICE_ROLE=api` — admin-URL не существуют, из nginx убрана. Код в репозитории остаётся (это часть проекта интернов), но на проде его нет |
...
- **Контента.** База пустая, админки нет — контент попадает в базу только
  импортом (`loaddata`/скрипт). Как именно — решает Макс.
